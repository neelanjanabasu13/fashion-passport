  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="approval-modal" role="dialog" aria-modal="true" aria-labelledby="approval-title">
        ...
        <p className="modal-lede">This single approval lets Fashion Passport apply your profile across compatible Shopify fashion stores, categories and queries, while your browsing history stays in this browser.</p>
      ...
      <div className="bound-book passport-bound-book">
        ...
        <section className="profile-groups">
          ...
          <div className="profile-layer theory-layer">
            ...
            <p>This starting point comes from your proportions and colouring and carries the lowest weight in every ranking.</p>
          ...
          <div className="profile-layer preference-layer">
            ...
            <p>Your stated choices outrank the foundation whenever the two disagree.</p>
            {stated.length === 0 && <p className="empty-layer">The taste pass will add preferences here as you react.</p>}
          ...
          <div className="profile-layer learned-layer">
            ...
            <p>Fashion Passport keeps learning as you shop, so the profile needs no regular maintenance.</p>
            {preferences.length === 0
              ? <p className="empty-layer">A learned pattern appears after repeated evidence across several reactions.</p>
          ...
          <div className="override-callout"><Icon name="sparkle" /><div><strong>Your taste has the greatest weight.</strong><p>Your choices overrule conflicting foundation guidance, while learned signals refine the order and keep every eligible product visible.</p></div></div>

          <div className="avoid-row">
            <strong>Held by your rules</strong>
            <div>{hardRules.length === 0 ? <span className="empty-rule">Your profile currently contains no Never rules that hold products.</span> : hardRules.map(({ title, value }) => <span key={`${title}-${value}`}>✕ {value}</span>)}</div>
          </div>
        </section>
        </div>
      </div>
    ...
    <main className="home-page">
      <section className="home-hero">
        <div className="home-hero-copy">
          <p className="eyebrow">Build once, use it across compatible shops</p>
      ...
      <section className="how-it-works">
        <div className="how-heading"><p className="eyebrow">How it works</p><h2>Build it once and let it improve as you shop.</h2></div>
        <div className="how-steps">
          ...
          <article><span>02</span><h3>Teach it your taste</h3><p>React to real clothes so your choices can overrule theory and sharpen future rankings.</p><button onClick={onboarded ? onPassport : onBuild}>{onboarded ? "See my profile" : "Build my profile"} →</button></article>
          <article><span>03</span><h3>Use it where you shop</h3><p>Connect once to rank live products here and on compatible retailers’ own websites.</p><button onClick={onShop}>Shop with my Passport →</button></article>
        </div>
      </section>
...
function TravelView({ connected, onConnect, onCompare }: { connected: boolean; onConnect: () => void; onCompare: () => void }) {
  ...
  return (
    <main className="travel-page">
      <section className="travel-hero"><p className="eyebrow"><Icon name="sparkle"/> Store-to-store portability</p><h1>One Fashion Passport to shop them all.</h1><p>When you open a compatible Shopify store, Fashion Passport discovers its official endpoint and applies the same size, taste and suitability profile on the retailer’s own website.</p>
      ...
      <section className="travel-demo">
        <div className="travel-demo-copy"><p className="eyebrow">Store-to-store demonstration</p><h2>The same Passport works at Jigsaw and Lucy &amp; Yak.</h2><ol><li>Reload the unpacked extension once.</li><li>Open Jigsaw and use Fashion Passport on the live site.</li><li>Open Lucy & Yak, where the same approved Passport is already available.</li></ol><button className="text-button" onClick={onCompare}>Compare all verified test stores →</button></div>
      ...
      <section className="any-store"><div><p className="eyebrow">Runtime store discovery</p><h2>Try another Shopify fashion store</h2><p>The extension checks the store you choose and appears when it finds a compatible catalogue tool.</p></div><form onSubmit={openStore}><input value={storeUrl} onChange={(event) => setStoreUrl(event.target.value)} placeholder="shop.example.com" aria-label="Shopify store URL"/><button>Open store <Icon name="arrow"/></button></form></section>
      <section className="verified-destinations"><div><p className="eyebrow">Live test panel</p><h2>{retailers.length} destinations we directly checked</h2><span>Runtime discovery extends the same adapter beyond this verification panel.</span></div><div>{retailers.map((store) => <a key={store.id} href={store.url} target="_blank" rel="noreferrer"><strong>{store.name}</strong><small>Official UCP endpoint verified</small><Icon name="external"/></a>)}</div></section>
    </main>
  );
}
...
function TasteView({ profile, onProfile, onDone }: { profile: FashionProfile; onProfile: (profile: FashionProfile) => void; onDone: () => void }) {
  ...
  const react = (reaction: Reaction) => {
    ...
    if (!broadened && nextTotal >= 6 && nextDislikes / nextTotal > 0.5) {
      ...
      setReactionNote(reaction === "down" && fitsTheory ? "Your preference now overrides this suitability suggestion." : reaction === "up" ? "The reaction is saved, and the profile will change after a repeated pattern appears." : "The reaction is saved without blacklisting every feature on this item.");
    }
    setIndex((n) => n + 1);
  };
  const bodyOptions = [
  ...
  return (
    <main className="onboarding-page verdict-onboarding">
      <section className="verdict-intro-copy">
        ...
        <p className="verdict-lede">Build a portable verdict from what is likely to suit you and what you genuinely love, then use it across compatible fashion stores without starting again.</p>
      ...
      {stage === "taste" && <><div className="page-heading compact"><p className="eyebrow">{broadened ? "Taste-led mode" : "Analysis-led mode"} · {sources || "multiple"} live retailers</p><h1>Teach Fashion Passport your taste</h1><p>Twenty rapid reactions reveal patterns across colour, silhouette, neckline, sleeve, length, fabric and print, while the deck broadens automatically when you reject more than half of its first choices.</p></div><section className="taste-stage">
        ...
        {loading ? <div className="taste-loading">Matching live products to your body and colour analysis…</div> : current ? <>
          ...
          <p className="microcopy">{reactionNote || "This reaction stays local, and the taste pass will move to another real product."}</p>
...
function PrivacyView({ connected, onRevoke }: { connected: boolean; onRevoke: () => void }) {
  return (
    <main className="secondary-page privacy-page">
      <div className="page-heading"><p className="eyebrow">Privacy by default</p><h1>Your Passport stays under your control.</h1><p>You approve it once, can pause it anywhere and can disconnect it here.</p></div>
      <div className="privacy-grid">
        <section className="privacy-principles">
          <article><Icon name="shield"/><div><h3>Photos disappear</h3><p>Uploads are processed temporarily, with only the colour season or body-shape result retained.</p></div><span>Always on</span></article>
          <article><Icon name="lock"/><div><h3>Learning stays here</h3><p>Browsing, likes and skips remain in this browser while cross-device sync stays off.</p></div><span>Local</span></article>
      ...
      const tools = [
        {
          ...
          description: "Returns the shopper's stable fashion profile, including size, budget, colour season, body shape, and loved or avoided garment attributes for use before fashion search or ranking.",
        ...
        },
        {
          ...
          description: "Searches every verified live Shopify UCP catalogue with one request and returns a cross-store personal ranking that gives explicit preferences greater weight than styling theory.",
        ...
        },
        {
          ...
          description: "Opens the single visible Fashion Passport consent screen and waits for the shopper to approve in the interface.",
          inputSchema: { type: "object", properties: {}, additionalProperties: false },
          execute: async () => {
    ...
    <div className="app-shell">
      ...
      {view === "shop" && <main className="shop-page">
        <section className="hero-copy"><p className="eyebrow"><Icon name="sparkle" /> One Passport across Shopify fashion</p><h1>Stop starting from scratch.</h1><p>One standard adapter carries your size, taste and suitability context into any compatible Shopify fashion store.</p>
          <div className="scale-proof"><strong>{retailers.length}</strong><span>retailers verified against their own UCP endpoint</span><i></i><strong>1 adapter</strong><span>runtime compatibility discovery across stores</span></div>
        ...
        <aside className="surface-explainer"><Icon name="passport"/><div><strong>Two ways to use it</strong><span>This hub compares stores, while the extension opens the Passport panel on a brand’s own website after you choose “Use on real store”.</span></div></aside>
